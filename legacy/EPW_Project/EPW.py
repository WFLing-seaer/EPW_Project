...
class Storage:
    ...
    def flatten(self, cls) -> str:
        ...
        for val in dir(cls):  # 遍历类内的所有函数/变量/...
            if val[0] == val[1] == val[-2] == val[-1] == "_":
                continue  # 初步筛除私有变量/函数
            xval = getattr(cls, val)  #
            sval = str(type(xval))  # 获取xval的类型
            if sval in ("<class 'function'>", "<class 'method'>"):
                continue  # 不保存函数
            ...
            def iscontainer(e) -> bool:  # 是否为容器
                try:
                    ...
                    return bool(getattr(e, "__iter__", False))

            def run(lst, func) -> Any:  # 对嵌套列表等容器内的每一项执行函数
                print("run run on", lst)
                DCT, TPL = isinstance(lst, dict), isinstance(lst, tuple)
                if DCT:
                    lst = list(lst.items())  # 将映射、元组展开为列表方便操作
                if TPL:
                    lst = list(lst)
                for ind in range(len(lst)):
                    if iscontainer(lst[ind]):
                        lst[ind] = run(lst[ind], func)
                    else:
                        lst[ind] = func(lst[ind])
                return dict(lst) if DCT else tuple(lst) if TPL else lst

            print("val", xval, sval, iscontainer(xval))

            if sval == "<class 'type'>":
                res += repr(self.flatten(xval))  # 展开嵌套类
            elif iscontainer(xval):
                res += repr(run(xval, str))
            else:
                res += repr(xval)

            res += ","  # 分割字典项
        return (
            "(" + repr(repr(cls)[repr(cls).index(".") + 1 : repr(cls).index(" ")]) + "," + res[:-1] + "})"
        )  # 去除多余逗号，并带上类名
...
class world(Storage):
    ...
    class player(Storage):
        ...
        def render(self, shiver=None) -> Image:
            ...
            while True:
                try:
                    ...
                    pixelpopularity = min(
                        (sum([plc[2][0] * plc[2][1] for plc in world.places[self.pos].pixels]) // len(herepixel)),
                        min([min(plc[2][0], plc[2][1]) for plc in world.places[self.pos].pixels]) ** 2 / 4,
                    )  # 计算像素点密度（懒得搞二维均衡了，偷懒按照正方形计算了）（悲）
                    pixelplates = deepcopy(world.places[self.pos].pixels)  # 能塞(sei)像素点的地方
                    prt = {}  # 像素点偏移量
                    if shiver and len(shiver) / len(herepixel) != 2:
                        raise self.RenderError("无法渲染像素点：无效的抖动帧")  # shiver长度不对就抛错
                    for ignore in range(len(herepixel)):
                        seed(tuple(prt))
                        plate = choice(
                            pixelplates
                        )  # 瞎选一个，这里用seed的原因是要保证渲染各帧的时候像素点不会乱串（因为每次渲染的时候所有变量都一样所以不会串）
                        prt[plate] = prt.get(plate, (0, 0))  # 获取偏移量，或者设置默认值0,0
                        if hpsize == -1:
                            hpsize = floor(pixelpopularity**0.5)
                        psize = hpsize  # 计算像素点的大小
                        if psize < 2:
                            raise self.RenderError("无法渲染像素点：密度过大")  # 检验
                        pgrid = plate[2][0] // psize
                        phgrid = plate[2][1] // psize  # 这两行是计算一片区域内排像素点的行列数
                        prt[plate] = (prt[plate][0] + 1, prt[plate][1])  # 下一个格点
                        if prt[plate][0] >= pgrid:
                            prt[plate] = (0, prt[plate][1] + 1)  # 同上
                        try:
                            if prt[plate][1] >= phgrid:
                                pixelplates.remove(plate)  # 塞满一个之后就不要塞了
                        except ValueError:
                            raise self.RenderError("无法渲染全部像素点：可渲染面积不足")  # 所有plate都塞满了像素点还没排完
                    break
                except IndexError:
                    # print('dec size:',hpsize)
                    hpsize -= 1
            BG = Image.new(
                "RGB", world.places[self.pos].BGimg.size, (0, 0, 0)
            )  # 创建一个背景图的空副本（因为deepcopy不能复制图像迫不得已才出此下策）
            ...
            [
                BG.paste(
                    ...
                    .mipmap[MIPMAP]
                    .resize(
                        (int(item[0].mipmap[MIPMAP].size[0] * item[3]), int(item[0].mipmap[MIPMAP].size[1] * item[3])),
                        resample=RESAMPLE,
                    ),
                    item[1:3],
                    None,
                )
                for item in world.places[self.pos].items
            ]  # 粘物品
            herepixel = world.places[self.pos].herepixel  # 在此的像素点
            pixelpopularity = min(
                (sum([plc[2][0] * plc[2][1] for plc in world.places[self.pos].pixels]) // len(herepixel)),
            ...
            if shiver and len(shiver) / len(herepixel) < 2:
                ...
                shiver = shiver[: len(herepixel) * 2]
            _shiver = iter(
                shiver or [randint(0, 5) for ignore in range(2 * len(herepixel))]
            )  # 没有shiver就弄个空shiver上去（shiver的格式是，每两个值表示一个像素点的抖动，因为在渲染的时候herepixel不会变，所以不用担心对不齐的问题）
            while True:
                seed(tuple(prt))
                plate = choice(
                    pixelplates
                ...
                try:
                    ...
                    ImageDraw.Draw(pxl).text(
                        (0, 0),
                        pxlb.name,
                        tuple((c + 128 if c <= 127 else c - 128 for c in pxlb.color)),
                        ImageFont.truetype("MAE.ttf", max(2, min(psize // len(pxlb.name), psize // 6))),
                    )
                    BG.paste(
                        pxl,
                        (
                            int(plate[0] + prt[plate][0] * psize + (psize * next(_shiver) / 100)),
                            int(plate[1] + prt[plate][1] * psize - (psize * next(_shiver) / 100)),
                        ),
                        None,
                    )  # 粘像素点，-2是为了抖动
        ...
        def move(self, to, forced=False) -> None:
            if not (to in world.places.keys()):
                raise self.UnknownPlaceError
            if not (forced or (to in world.places[self.pos].conn)):
                raise self.InvalidPlaceError
            try:
    ...
    class place(Storage):
        ...
        def __init__(
            ...
            ID: int,
            desc: str = "未命名地点|暂无简介",
            BGimg: Image = mipmap.mipmaps["unknown"],
            conn: list[int] = [],
            pixels: list[tuple[int, int, tuple[int, int]]] = [(0, 0, (128, 128))],
            items: list[tuple[type, int, int, int]] = [],
        ) -> None:
            self.ID = ID  # ID                     （整型）
            self.desc = desc  # 名称|描述              （字符串）
...
class backup:
    def dump() -> None:
        with open(Dumpfile, "wb+") as dumpfile:
            pickle.dump((world.players, world.places, world.users, world.items), dumpfile)
        """with open(TDumpfile,'w') as dumpfile:
            try:dumpfile.write(Storage().flatten(scope))
            except:..."""

    def load() -> None:
        with open(Dumpfile, "rb+") as dumpfile:
            world.players, world.places, world.users, world.items = pickle.load(dumpfile)
