...
class FUNC:
    ...
    def 前往(msg, *info):
        if info:
            ...
            return (
                "当前您位于"
                + "（".join(EPW.world.places[current_pixel[int(msg.author.id)].pos].desc.split("|"))
                + "）\n当前可以前往的地点列表:\n编号\t地点\n"
                + "\n".join(
                    [
                        str(k) + "\t" + EPW.world.places[k].desc.split("|")[0]
                        for k in EPW.world.places[current_pixel[int(msg.author.id)].pos].conn
                    ]
                )
            )
...
注意：注册时将会从像素点.cn查询像素点信息，可能需要一分钟左右时间，请耐心等待。"""
            ...
            if not all(
                [len(c) == 7 and c[0] == "#" and all(["0" <= ch <= "9" or "A" <= ch <= "F" for ch in c[1:]]) for c in info]
            ):
                return "您的像素点格式有误！"
            ...
            for pixel in pixels:
                try:
                    ...
                    txt = txt.text.split("／")
                    if txt[0] in [
                        str(hex((a.color[0] << 16) + (a.color[1] << 8) + a.color[2]))[2:] for a in EPW.world.players.values()
                    ]:
                        return "E32:像素点已经注册"
            ...
            for txt in reversed(txts):
                print(repr(txt), "txt")
                current_pixel.update(
                    {
                        int(msg.author.id): EPW.world.player(
                            int(hashlib.md5(str(txt[0][0]).encode()).hexdigest(), base=16),
                            txt[0][0],
                            (int(txt[1][0:2], base=16), int(txt[1][2:4], base=16), int(txt[1][4:], base=16)),
                        )
                    }
                )
    ...
    def 切换像素点(msg, *info):
        if not info:
            return (
                "切换像素点：请输入像素点编号（需为您注册的，井号打头，大写字母）以切换到对应像素点。\n例：/切换像素点 #E4E4FC"
            )
        else:
            if info[0].lstrip("#") in [
                str(hex((p.color[0] << 16) + (p.color[1] << 8) + p.color[2]))[2:].upper()
                for p in EPW.world.users[int(msg.author.id)].players
            ]:
                current_pixel[int(msg.author.id)] = [
                    p
                    for p in EPW.world.users[int(msg.author.id)].players
                    if str(hex((p.color[0] << 16) + (p.color[1] << 8) + p.color[2]))[2:] == info[0].lstrip("#").lower()
                ][0]
                pickle.dump(current_pixel, open("~$pxtmp", "wb+"))
...
class main(botpy.Client):
    async def on_at_message_create(self, message: botpy.message.Message) -> None:
        ...
        try:
            func = getattr(
                ...
                message.content[
                    ...
                    + 1 : message.content.index(" ")
                    + message.content[message.content.index(" ") + 1 :].index(" ")
                    + 1
                ],
            )
            args = message.content[
                message.content.index(" ") + message.content[message.content.index(" ") + 1 :].index(" ") + 2 :
            ].split()
        except Exception as e:
        ...
        await message.reply(
            content=ERR or (output if isinstance(output, str) else output[0]),
            file_image=output[1] if isinstance(output, tuple) else None,
        )
